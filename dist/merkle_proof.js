System.register(["./const", "./errors/errors", "./h256", "./merge/merge", "./merge/util"], function (exports_1, context_1) {
    "use strict";
    var const_1, errors_1, h256_1, merge_1, util_1, MerkleProof, CompiledMerkleProof;
    ...
    return {
        ...
        execute: function () {
            ...
            CompiledMerkleProof = class CompiledMerkleProof extends Array {
                compute_root(leaves) {
                    let stack = new Array;
                    leaves.sort();
                    let leaf_index = 0;
                    ...
                    while (pc < this.length) {
                        ...
                        switch (code) {
                            case 0x4c:
                                ...
                                stack.push([0, leaves[leaf_index][0], util_1.from_h256(leaves[leaf_index][1])]);
                                leaf_index++;
                                break;
                            case 0x50:
                                if (stack.length == 0) {
                                ...
                                let data = new h256_1.H256(this.slice(pc, pc += 32));
                                sibling = util_1.from_h256(data);
                                ...
                                parent = (() => {
                                    if (key.is_right) {
                                        return util_1.merge(height, parent_key, sibling, value);
                                    }
                                    else {
                                        return util_1.merge(height, parent_key, value, sibling);
                                    }
                                })();
                            ...
                            case 0x51:
                                ...
                                zero_count = this[pc++];
                                let base_node = new h256_1.H256(this.slice(pc, pc += 32));
                                let zero_bits = new h256_1.H256(this.slice(pc, pc += 32));
                                [height, key, value] = stack.pop();
                                if (height > const_1.MAX_HEIGHT) {
                                    throw new errors_1.CorruptedProofError;
                                }
                            ...
                            case 0x48:
                                ...
                                parent = (() => {
                                    if (key_a.is_right(height_a)) {
                                        return util_1.merge(height_a, parent_key, value_b, value_a);
                                    }
                                    else {
                                        return util_1.merge(height_b, parent_key, value_a, value_b);
                                    }
                                })();
                            ...
                            case 0x4f:
                                ...
                                for (let i = 0; i < n; i++) {
                                    ...
                                    value = ((height) => {
                                        parent_key = key.parent_path(height);
                                        if (key.is_right(height)) {
                                            return util_1.merge(height, parent_key, new merge_1.MergeValueNormal(h256_1.H256.zero()), value);
                                        }
                                        else {
                                            return util_1.merge(height, parent_key, value, new merge_1.MergeValueNormal(h256_1.H256.zero()));
                                        }
                                    })(height + i);
                                }
